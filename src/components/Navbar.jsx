...
export default function Navbar({ activePage, setActivePage, onOpenRegister, theme, toggleTheme }) {
  ...
  return (
    <>
        ...
        <div className="flex justify-between items-center w-full px-gutter py-3 max-w-container-max mx-auto">
            ...
            {navLinks.map((link) => (
              <button
                ...
                className={`relative py-1 transition-all duration-200 inline-block hover:scale-105 active:scale-110 ${
                  activePage === link.id || (link.id === 'landing' && (activePage === 'journey' || activePage === 'speakers-section'))
          ...
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            <button
              ...
              className="md:hidden text-on-surface dark:text-dark-text p-2 focus:outline-none transition-transform duration-200 hover:scale-110 active:scale-120"
      ...
      {/* Mobile Drawer */}
      <div
        className={`fixed top-0 left-0 bottom-0 w-72 bg-surface dark:bg-dark-surface z-50 border-r border-outline-variant/30 dark:border-dark-border shadow-2xl transform transition-transform duration-300 ease-in-out md:hidden flex flex-col py-6 px-6 ${
          mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex items-center justify-between pb-6 mb-6 border-b border-outline-variant/20 dark:border-dark-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body-md text-sm text-on-surface-variant dark:text-dark-text-muted">Fall Fest 2026</p>
            </div>
          </div>
          <button 
            onClick={() => setMobileMenuOpen(false)}
            className="text-on-surface-variant dark:text-dark-text-muted hover:text-primary transition-transform duration-200 hover:scale-110 active:scale-120"
          >
